...
//   www.plexoninc.com
//
//   Simple dialog-based MFC app that shows how to read and interpret VT coordinate data.
//
...
void CMFCVTReadDlg::OnTimer(UINT nIDEvent) 
{
    ...
    if (bFetchingTimeStamps && (nIDEvent == nTimerID))
    {
      ...
      buf.Format("%d", NumMAPEvents);
      SetDlgItemText(IDC_EVENTSREAD, buf);

      //** step through the array of MAP events, processing only the strobed event timestamps
      for (int i = 0; i < NumMAPEvents; i++)
      {
